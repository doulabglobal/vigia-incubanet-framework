...
```mermaid
flowchart LR
    A(Outreach & Attraction) --> B(Selection & Eligibility)
    B --> C(Incubation Delivery)
    C --> D(Acceleration & Investment Readiness)
    D --> E(Investment & Tranching)
    E --> F(Monitoring · KPIs · MEL)
    F --> G(Graduation)
    G --> H(Foresight & Policy Feedback)
    H --> A
```
...
```mermaid
flowchart TD
    A(Startups) --> B(Incubator Nodes)
    B --> C(TOU  -  Evidence Review)
    C --> D(Investment Committee)
    D --> E(Tranche Decision)
    E --> F(MEL · KPIs · Dashboard)
    F --> G(Foresight Cycle · Vigía Futura)
    G --> A
```
