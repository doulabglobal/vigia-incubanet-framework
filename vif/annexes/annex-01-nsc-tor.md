...
```mermaid
flowchart TD
    A[Quarterly Reports Submitted] --> B[TOU Prepares Briefing]
    B --> C[NSC Meeting Convened]
    C --> D{Topics?}
    D -->|KPIs| E[Review Scorecards]
    D -->|Funding| F[Approve Allocations]
    D -->|Governance| G[Validate Policy Updates]
    E --> H[Decisions Recorded]
    F --> H
    G --> H
    H --> I[Public Summary Published]
```
...
```mermaid
flowchart LR
    A[Q1 KPI Review] --> B[Sector Signal Update]
    B --> C[Policy Adjustment Draft]
    C --> D[Funding Allocation Approval]
    D --> E[Implementation Monitoring]
    E --> F[Annual Review & Foresight Report]
    F --> A
```
